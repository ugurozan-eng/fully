...
* **Raporlar (CRM Raporları)**: Rapor leads tablosu üzerinde Excel tipi filtreleme ve sıralama sistemi. Tarih filtreleri ("Tüm Zamanlar", "Bugün", "Dün", "Bu Hafta", "Bu Ay", "Tarih Aralığı") ile veriyi süzme. Başlığın yanındaki "Excel'e Aktar" butonu ile filtrelenmiş ve sıralanmış güncel verileri otomatik kolon genişlikleri (Red Nedeni kolonu dahil) ve sayısal bütçe formatı ile doğrudan `.xlsx` formatında dışa aktarma. Tablolardaki popover filtrelerinin tıklandığında aniden kapanması engellenmiştir (stopPropagation).
